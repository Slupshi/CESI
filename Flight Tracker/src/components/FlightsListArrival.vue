<script>
import FlightsListArrivalItem from './FlightsListArrivalItem.vue'

export default {
    name : "FlightsList",
    components: {
    FlightsListArrivalItem,
    }, 
    data(){
        return {
            arrivals : [],
...
async function GetArrivals(){  
    ...
    let url = `https://opensky-network.org/api/flights/arrival?airport=${icao}&begin=${begin}&end=${end}`; 
...
<template>
    <div>
        <p v-for="item in arrivals" :key="item.icao24">
                <!-- <FlightsListArrivalItem :Arrival="this.DateConverter(item.firstSeen)" :Airport="this.GetAirportName(item.estDepartureAirport)"/> Problème de 403 au bout d'un certain nombre de try // Call Limit atteinte-->
                <FlightsListArrivalItem :Arrival="this.DateConverter(item.firstSeen)" :Airport="item.estDepartureAirport"/> 
        </p>                
    </div>
</template>
